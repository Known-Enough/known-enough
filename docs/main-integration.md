...
History-preserving local merge `2dd036a` retains both lines as parents. The current tree selects the stricter multi-item STATE/GUARD/REPLAY adapter and authenticated API composition, removes the duplicate single-item adapter, and ports the published line’s opt-in DynamoDB Local scenario to the selected adapter. Old reviews remain tied to their exact pre-merge artifacts. The unified artifact requires fresh independent B04.5 review. A later explicit user request pushed the unified history and review records to `origin/main` at `4616b42`; subsequent B04 emulator verification is local and has not been pushed.


Follow-up review of `2dd036a` found one P2: package-level exports for the Cognito resolver were missing, and the generated lock omitted registry integrity metadata while the SDK pin did not match installed modules. B04 restored the package exports with a package-surface regression, aligned SDK 3.1135.0 with installed modules, and restored complete lock metadata. The corrected code passed full local checks and fresh independent Astra/high follow-up on exact code commit `a058cc5` passed. B04's migrated DynamoDB Local scenario subsequently passed on loopback 3.3.1, and the full local check passed with the emulator enabled. A later acceptance audit found that B04's “invite replay” criterion was not represented by any room-invitation route. B04 added subject-bound, single-use invitation issuance/redemption in local commit `13707c2`; that new security slice is PAUSED for fresh independent B04.5 follow-up. No managed-service/live acceptance is claimed; these verification/status records are local and have not been pushed.
