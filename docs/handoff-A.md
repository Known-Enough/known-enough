...
Current scheduling (September 24): both users follow the same [project-wide priority queue](task-board.md), with one active task at a time. G01 is DONE for its accepted local checkpoint. B04 invitation issuance/redemption is implemented in local main commit `13707c2` from baseline `3013297` and is PAUSED for fresh independent B04.5 review of the exact code. Focused application/signed-Cognito HTTP/repository suites, the full project check and the opt-in DynamoDB Local competing-redemption test passed; evidence is in B04 and the A log. Actual worker was GPT-6 Codex with variant/effort unexposed, not claimed as the ticket's `gpt-6-sol` / high target. No managed cloud acceptance, deployment, push or human acceptance is claimed.
