...
Use the direct model/effort printed in the [ticket](task-board.md); no Astra manager is required. A02 is now in REVIEW; choose A03/A06 (Luna medium) as ready alternatives. Read [workflow](agent-workflow.md) and [A log](work-log-A.md), then claim one task.
