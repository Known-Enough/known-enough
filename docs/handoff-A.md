...
F00–F02 establish the review candidate foundation: npm workspaces, public React scaffold, Zod contract v1, examples, canonical public SHA-256, import boundaries and credential-free CI. See README and docs/verification.md. The bootstrap was originally created as local uncommitted work; the existing review baseline is commit `972386395618ec0ed81581d38082ebeb15d66142` on `main`. It remains REVIEW pending human acceptance. Share the accepted baseline with B before separate task branches.
...
Acceptance: mock shared/owner flows, safe error/stale handling, no private values in public snapshots, keyboard/mobile checks plus npm run check. The dated bootstrap record reports 49 unit tests, 2 Chromium tests, 15 arithmetic checks, lint/typecheck/build and 7 reference checks passed; rerun the required checks against the current review state. Backend identity, expiry and concurrency remain unimplemented.
