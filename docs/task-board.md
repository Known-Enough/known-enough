...
- **Current gate: B04.5** (`gpt-6-astra`, high), independent review of B04’s first Cognito identity slice and persistence/IAM design. B04 is PAUSED at this midpoint; address review findings before resuming implementation. G01’s local integration checkpoint received independent follow-up PASS and user acceptance on 2026-09-23.
...
Task prerequisites determine eligibility; the project-wide priority above determines order. B04 is paused while its mandatory midpoint review is next. B03 acceptance still requires review of B02's final critical delta. Integration and human review cannot be replaced with synthetic browser tests.
...
| [B04](tasks/B04.md) | `gpt-6-sol` / high | G01 | PAUSED; first Cognito identity slice ready for midpoint review |
| [B04.5](tasks/B04.5.md) | `gpt-6-astra` / high | G01 + B04 design/first implementation | READY; independent midpoint review of identity, transactions and IAM design |
...
A04/A05 preparation does not technically depend on G01, and A03.5 passes for the exact recorded artifact. G01 is DONE for the reviewed local checkpoint; B04 is paused at the B04.5 review gate. A05 mock preparation remains REVIEW pending its separate human acceptance. A04 still requires an agreed session interface. B04 must pause for B04.5 before its critical implementation continues. External testers still require G02 and operational readiness.
