...
- **Current task: B04 implementation** (`gpt-6-sol`, high). Independent B04.5 follow-up [PASSed the local midpoint design on `5297118`](reviews/B04.5.md), closing R5b; R6 and earlier closures remain valid. Owner A has resumed the DynamoDB codec/transaction implementation under the ticket. Adapter tests, critical code review, human acceptance and G02/live checks remain required. G01 received independent follow-up PASS and user acceptance on 2026-09-23.
...
| [B04](tasks/B04.md) | `gpt-6-sol` / high | G01 | IN_PROGRESS; strict DynamoDB codec/transaction implementation and adapter tests |
| [B04.5](tasks/B04.5.md) | `gpt-6-astra` / high | G01 + B04 design/first implementation | REVIEW / PASS on 5297118 for local midpoint design; claim finished |
...
A04/A05 preparation does not technically depend on G01, and A03.5 passes for the exact recorded artifact. G01 is DONE for the reviewed local checkpoint; B04 is IN_PROGRESS after B04.5 PASSed its local design checkpoint. Adapter enforcement and near-limit/concurrency tests precede final B04 review. A05 mock preparation remains REVIEW pending its separate human acceptance. A04 still requires an agreed session interface. External testers still require G02 and operational readiness.
