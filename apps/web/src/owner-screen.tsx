...
import { HOST_LANGUAGE_SAMPLE, hostSimulationExtractor, type OwnerDraftExtractor, validateOwnerDraftResponse } from './owner-draft-extractor';
...
export function OwnerScreen({ initialScenario, localIdentity = null, draftExtractor = hostSimulationExtractor }: { initialScenario: string | null; localIdentity?: Exclude<LocalIdentity, 'display'> | null; draftExtractor?: OwnerDraftExtractor }) {
  ...
  const [languageText, setLanguageText] = useState(HOST_LANGUAGE_SAMPLE);
  const [languageDraft, setLanguageDraft] = useState<InputValues | null>(null);
  ...
  const invalidateLanguageDraftReview = () => { setReviewedIntervals([]); setDraftReviewInvalidated(true); };
  const prepareLanguageDraft = async () => {
    if (!publicRoom) return;
    invalidateLanguageDraftReview();
    setExtracting(true); setLanguageError(false); setLanguageDraft(null);
    try {
      const parsed = validateOwnerDraftResponse(await draftExtractor(languageText, publicRoom));
      if (!parsed) throw new Error('Invalid draft response');
      setLanguageDraft(parsed);
    } catch {
  ...
  return <main className="owner-page">
    ...
    {room && <div className="owner-grid">
      <section className="owner-card" aria-labelledby="inputs"><p className="eyebrow">OWNER REVISION {room.ownerRevision}</p><h2 id="inputs">Your inputs</h2><p>Keep reasons private. Record a condition or preference without explaining why.</p>{!localIdentity && publicRoom && <section className="language-draft" aria-labelledby="language-draft-title"><p className="eyebrow">HOST SIMULATION</p><h3 id="language-draft-title">Review a language draft</h3><p>This fixed local example supports the sentence shown. It does not call a model or service, and the sentence stays in this browser demo. Other wording returns to the structured form below.</p><label htmlFor="language-draft-text">Private sentence</label><textarea id="language-draft-text" value={languageText} disabled={disabled || extracting} onChange={event => { setLanguageText(event.target.value); setLanguageDraft(null); setLanguageError(false); invalidateLanguageDraftReview(); }} /><div className="button-row"><button type="button" disabled={disabled || extracting || !languageText.trim()} onClick={() => void prepareLanguageDraft()}>{extracting ? 'Preparing sample…' : 'Prepare simulated draft'}</button><button type="button" className="secondary" disabled={disabled || extracting} onClick={() => { setLanguageText(HOST_LANGUAGE_SAMPLE); setLanguageDraft(null); setLanguageError(false); invalidateLanguageDraftReview(); }}>Restore sample sentence</button></div>{languageError && <p role="alert">No safe sample response is available for that wording. Use the structured form below or restore the sample sentence.</p>}{languageDraft && <div className="suggested-draft" role="region" aria-label="Suggested private draft"><h4>Suggested draft — still unconfirmed</h4><InputSummary values={languageDraft} room={publicRoom} /><p>No duty costs were provided or inferred.</p><div className="button-row"><button type="button" disabled={disabled} onClick={() => { setReviewedIntervals([]); setDraftReviewInvalidated(true); void run(submitInputDraft(room, context, languageDraft)); }}>Submit this private draft</button><button type="button" className="secondary" disabled={disabled} onClick={() => setLanguageDraft(null)}>Use structured form instead</button></div></div>}</section>}{localIdentity && publicRoom && !inputValues ? <InitialInputForm key={publicRoom.contextToken} room={publicRoom} disabled={disabled} submit={next => { setReviewedIntervals([]); setDraftReviewInvalidated(true); void run(submitInputDraft(room, context, next)); }} /> : <fieldset disabled={disabled}><legend>Meeting availability</legend><label><input type="radio" name="availability" checked={availability === 'exception'} onChange={() => { setAvailability('exception'); setReviewedIntervals([]); setDraftReviewInvalidated(true); }} /> {selectedInterval ? `${selectedInterval.date} · ${time(selectedInterval.startMinute)}–${time(selectedInterval.endMinute)} · ${selectedInterval.timezone} is unavailable, but you may ask about a scoped exception.` : 'A scoped availability question is available.'}</label><label><input type="radio" name="availability" checked={availability === 'available'} onChange={() => { setAvailability('available'); setReviewedIntervals([]); setDraftReviewInvalidated(true); }} /> {selectedInterval ? `${selectedInterval.date} · ${time(selectedInterval.startMinute)}–${time(selectedInterval.endMinute)} · ${selectedInterval.timezone} is available.` : 'This exact interval is available.'}</label>{localIdentity && <label><input type="radio" name="availability" checked={availability === 'unavailable'} onChange={() => { setAvailability('unavailable'); setReviewedIntervals([]); setDraftReviewInvalidated(true); }} /> {selectedInterval && describeInterval(selectedInterval)} is unavailable; do not ask for an exception.</label>}<label>Meeting duration <select value={duration} onChange={event => { setDuration(event.target.value); setReviewedIntervals([]); setDraftReviewInvalidated(true); }}><option value="30">30 minutes</option><option value="60">60 minutes</option></select></label><label>Follow-up duty cost <select value={cost} onChange={event => { setCost(event.target.value); setReviewedIntervals([]); setDraftReviewInvalidated(true); }}><option value="0">0 — no added inconvenience</option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select></label><button type="button" onClick={() => { const next = values(); if (next) { setReviewedIntervals([]); setDraftReviewInvalidated(true); void run(submitInputDraft(room, context, next)); } }}>Submit input draft</button></fieldset>}<p className="timezone">Submitting creates a versioned private draft; it does not grant an exception, authorize disclosure, or accept a plan.</p></section>
