...
function applicationError(error: unknown, id: string): ErrorResult {
  return error instanceof ApplicationError
    ...
    : errorBody('RETRYABLE_SERVER_ERROR', id);
}
  ...
  return (request, response) => {
    void (async () => {
      ...
      try {
        ...
        const result = applicationError(error, responseId);
...
function cognitoApiHandler(
  options: CognitoApiOptions,
  jwksCache?: JwksCache,
): (request: IncomingMessage, response: ServerResponse) => void {
...
export function createCognitoApiHandler(options: CognitoApiOptions): (request: IncomingMessage, response: ServerResponse) => void {
  return cognitoApiHandler(options);
}

/** @internal Local-signed-JWKS test seam; not re-exported from the package entrypoint. */
export function createCognitoApiHandlerWithJwksCache(
  options: CognitoApiOptions,
  jwksCache: JwksCache,
): (request: IncomingMessage, response: ServerResponse) => void {
  return cognitoApiHandler(options, jwksCache);
}
