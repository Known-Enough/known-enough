...
The codec allowlists and validates the version 3 private STATE envelope and exact STATE/GUARD/REPLAY keys. Roster revisions move departed owners’ exception grants and disclosure decision metadata into a private archive and preserve the lifetime permission-history counter. Archived disclosure records retain the preview hash but omit unshared wording; drafts and confirmations are not archived. Replay rows contain only hashes, incarnation, and a strict command result. Candidate receipts are supplied to the application only after the application callback authorizes against current membership. Pending exception/disclosure decisions reserve history and encoded STATE capacity; ordinary, permission-history, safety-reserve, and total receipt counters are guarded with every write. No TTL is used for consent or replay retention.
