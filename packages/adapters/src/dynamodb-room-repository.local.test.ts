...
import { DynamoDBRoomRepository } from './dynamodb-room-repository.ts';
...
describe.skipIf(!enabled)('DynamoDBRoomRepository against explicitly enabled DynamoDB Local', () => {
  beforeAll(async () => {
    ...
    document = DynamoDBDocumentClient.from(lowLevel);
    ...
    tableCreated = true;
    repository = new DynamoDBRoomRepository({ client: document, tableName, maxConflictRetries: 64 });
  }, 20_000);
  ...
  it('runs create, duplicate rejection, and competing read-modify-write transactions through the SDK', async () => {
    ...
    const stored = await lowLevel.send(new GetItemCommand({
      TableName: tableName,
      Key: { PK: { S: `ROOM#${roomId}` }, SK: { S: 'STATE' } },
      ConsistentRead: true,
    }));
    expect(Number(stored.Item?.storageVersion?.N)).toBe(12);
  });
});
